...
namespace System
{
    public sealed partial class Utf8String
    {
        ...
        private static readonly byte[] s_singleElementNullTerminatedArray = new byte[] { 0 };
        ...
        /// <summary>
        /// For internal <see cref="FastAllocate(int)"/> use only.
        /// </summary>
        private Utf8String() { }
        ...
        private void Ctor(ReadOnlySpan<byte> value)
        {
            // Create and populate the Utf8String instance.

            FastAllocateSkipZeroInit(this, value.Length);
            value.CopyTo(this.DangerousGetMutableSpan());

            // Now perform validation.
            // Reminder: Perform validation over the copy, not over the source.

            if (!Utf8Utility.IsWellFormedUtf8(this.AsBytes()))
            {
                throw new ArgumentException(
                    message: "Input contains ill-formed UTF-8.",
        ...
        public Utf8String(byte[] value, int startIndex, int length)
        {
            ...
            Ctor(new ReadOnlySpan<byte>(value, startIndex, length));
        }
        ...
        public unsafe Utf8String(byte* value)
        {
            ...
            Ctor(new ReadOnlySpan<byte>(value, strlen(value)));
        }
        ...
        private void Ctor(ReadOnlySpan<char> value)
        {
            ...
            if (newString is null)
            {
                ...
                throw new ArgumentException(
                    message: "Input contained ill-formed UTF-16 data.",
                    paramName: nameof(value));
            }

            // Copy data to our object and discard the intermediate.

            this._length = newString._length;
            this._nullTerminatedData = newString._nullTerminatedData;
        }
        ...
        public Utf8String(char[] value, int startIndex, int length)
        {
            ...
            Ctor(new ReadOnlySpan<char>(value, startIndex, length));
        }
        ...
        public unsafe Utf8String(char* value)
        {
            ...
            Ctor(new ReadOnlySpan<char>(value, wcslen(value)));
        }
        ...
        public static Utf8String Create<TState>(int length, TState state, SpanAction<byte, TState> action)
        {
            ...
            {
                throw new ArgumentException(
                    message: "Callback provided ill-formed UTF-8 data.",
        ...
        {
            if ((uint)length >= (uint)int.MaxValue)
            {
            ...
            {
                _nullTerminatedData = new byte[length + 1],
                _length = length
            };
        }
        ...
        /// The implementation of this method checks its input argument for overflow.
        /// </remarks>
        private static void FastAllocateSkipZeroInit(Utf8String instance, int length)
        {
            // TODO_UTF8STRING: Actually skip zero-init.
            ...
            instance._nullTerminatedData = new byte[length + 1];
            instance._length = length;
        }

        private static unsafe int strlen(byte* ptr)
        {
            for (byte* end = ptr; ; end++)
            {
                if (*end == 0)
                {
                    long byteLength = end - ptr;
        ...
        {
            for (char* end = ptr; ; end++)
            {
                if (*end == 0)
                {
                    long charLength = end - ptr;
                    return checked((int)charLength);
                }
            }
        }
    }
