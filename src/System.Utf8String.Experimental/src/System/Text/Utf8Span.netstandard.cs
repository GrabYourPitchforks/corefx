    ...
    public readonly ref partial struct Utf8Span
    {
        ...
        public Utf8Span this[Range range]
        {
            get
            {
                ...
                return UnsafeCreateWithoutValidation(Bytes[range]);
            }
        }
        ...
        {
            throw new NotSupportedException("Cannot call Utf8Span.Equals(object).");
        }
        ...
        public override int GetHashCode()
        {
            ...
            return Marvin.ComputeHash32(this.Bytes, Marvin.DefaultSeed);
        }
        ...
        internal unsafe string ToStringNoReplacement()
        {
            ...
            fixed (byte* pData = &MemoryMarshal.GetReference(Bytes))
            {
                ...
                char[] rented = ArrayPool<char>.Shared.Rent(utf16CharCount);
                try
                {
                    OperationStatus status = Utf8.ToUtf16(new ReadOnlySpan<byte>(pData, Length), rented, out _, out _, replaceInvalidSequences: false);
                    Debug.Assert(status == OperationStatus.Done, "Did somebody mutate this Utf8String instance unexpectedly?");
                    return new string(rented, 0, utf16CharCount);
                }
                finally
                {
                    ArrayPool<char>.Shared.Return(rented); // this is safe since we control all code in the 'try' block
                }
            }
        }
