...
namespace System.Text.Unicode
{
    internal static partial class Utf8Utility
    {
        ...
        private static unsafe nuint GetIndexOfFirstNonWhiteSpaceChar(ref byte utf8Data, nuint length)
        {
            ...
            fixed (byte* pUtf8Data = &utf8Data)
            {
                while (i < length)
                {
                    ...
                    else
                    {
                        ...
                        // AsPointer call below is safe because we've pinned the ref at method entry.

                        Rune.DecodeFromUtf8(new ReadOnlySpan<byte>(Unsafe.AsPointer(ref utf8Data), (int)length).Slice((int)i), out Rune decodedRune, out int bytesConsumed);
        ...
        private static unsafe nuint GetIndexOfTrailingWhiteSpaceSequence(ref byte utf8Data, nuint length)
        {
            // This method is optimized for the case where the input data is ASCII, and if the
            // data does need to be trimmed it's likely that only a relatively small number of
            // bytes will be trimmed.
            ...
            fixed (byte* pUtf8Data = &utf8Data)
            {
                while (length > 0)
                {
                    ...
                    else
                    {
                        ...
                        Rune.DecodeLastFromUtf8(new ReadOnlySpan<byte>(Unsafe.AsPointer(ref utf8Data), (int)length), out Rune decodedRune, out int bytesConsumed);
                        if (Rune.IsWhiteSpace(decodedRune))
                        {
