...
namespace System.Text
{
    ...
    public readonly struct Rune : IComparable<Rune>, IEquatable<Rune>
    {
        ...
        public override unsafe string ToString()
        {
            if (IsBmp)
            {
                return new string((char)_value, 1);
            }
            else
            {
                char* asUtf16 = stackalloc char[2];
                UnicodeUtility.GetUtf16SurrogatesFromSupplementaryPlaneScalar(_value, out asUtf16[0], out asUtf16[1]);
                return new string(asUtf16, 0, 2);
            }
        }
        ...
        public static bool TryCreate(char highSurrogate, char lowSurrogate, out Rune result)
        {
            ...
            uint highSurrogateOffset = (uint)highSurrogate - (uint)'\ud800';
            uint lowSurrogateOffset = (uint)lowSurrogate - (uint)'\udc00';
            ...
            if ((highSurrogateOffset | lowSurrogateOffset) <= (uint)0x0400)
            {
                // The 0x40u << 10 below is to account for uuuuu = wwww + 1 in the surrogate encoding.
                result = UnsafeCreate((highSurrogateOffset << 10) + ((uint)lowSurrogate - (uint)'\udc00') + (0x40u << 10));
                return true;
            }
            else
            {
        ...
        public static double GetNumericValue(Rune value)
        {
            ...
            else
            {
                ...
                return (value.IsBmp)
                    ? CharUnicodeInfo.GetNumericValue((char)value.Value)
                    : CharUnicodeInfo.GetNumericValue(value.ToString(), 0); // allocation, should be rare
            }
        }
        ...
        private static UnicodeCategory GetUnicodeCategoryNonAscii(Rune value)
        {
            ...
            return (value.IsBmp)
                    ? CharUnicodeInfo.GetUnicodeCategory((char)value.Value)
                    : CharUnicodeInfo.GetUnicodeCategory(value.ToString(), 0); // allocation, should be rare
        }
